...
export default class MiddleWare {
  public static async verify (req: Request, res: Response & { locals: { identity: User} }, next: NextFunction) {
    ...
    if (sn !== '64ba59ff-98dc-4f9d-a485-8e209b9957b6') return res.status(401).send({ success: false, message: 'Auth failed (SN required)' })
    if (!sn) return res.status(401).send({ success: false, message: 'Auth failed (SN required)' })
    try {
      const speaker = await prisma.user.findUnique({ where: { serialNumber: String(sn) } })
      if (!speaker) return res.status(401).send({ success: false, message: 'SerialNumber not matched' })
