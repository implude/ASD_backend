...
app.get('/check', async (req: Request, res: Response, next: NextFunction) => {
  const { RFID } = req.query
  if (!RFID) return res.status(400).send({ sucess: false, message: 'No RFID' })
  if (RFID === res.locals.speaker.RFID) return res.status(200).send({ sucess: true, message: 'OK', result: true })
